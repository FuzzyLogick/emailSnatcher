...
just Ctrl + A and then Ctrl + C that bad boy... the entire thing.  A copy of everything will be held in memory, on the clipboard. Next, run clipboardSnatcher to extract emails from the clipboard.  No need to do anything else, just enter ./clipboardSnatcher.py and it will pull emails from the clipboard and output them to the terminal.  But wait, there's more.  It will also automatically put the snatched emails back onto the clipboard for you to paste them into a text file for later.  Based on the work by Al Sweigart.
